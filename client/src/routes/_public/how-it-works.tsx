  ...
  return (
    <div className="flex w-full flex-col items-center relative px-4 pt-24 pb-32">
      ...
      <Particles
        ...
        refresh
      />
      <div className="fixed top-1/4 left-1/4 w-[700px] h-[700px] bg-indigo-500/5 dark:bg-indigo-500/8 blur-[160px] rounded-full pointer-events-none z-0" />
      <div className="fixed bottom-1/4 right-1/4 w-[600px] h-[600px] bg-blue-500/5 dark:bg-blue-500/8 blur-[150px] rounded-full pointer-events-none z-0" />

      {/* === HERO === */}
      <div className="container relative z-10 max-w-4xl mx-auto text-center mb-24">
        ...
        <BlurFade delay={0.3} offset={20}>
          <p className="max-w-2xl mx-auto text-lg md:text-xl text-black/60 dark:text-white/60 leading-relaxed">
                ...
                <BlurFade key={step.number} delay={0.1 + i * 0.08} offset={30}>
                  <div className={`flex flex-col ${isEven ? "md:flex-row" : "md:flex-row-reverse"} items-center gap-10 md:gap-16`}>
                    ...
                    <div className={`flex-1 ${isEven ? "md:text-right" : "md:text-left"} text-left`}>
                      ...
                      <h2 className="text-2xl md:text-3xl font-extrabold tracking-tight mb-3">{step.title}</h2>
                      <p className="text-black/60 dark:text-white/60 text-base leading-relaxed mb-5">{step.description}</p>
                      ...
                      {step.nodes && (
                        <div className="mt-6 grid grid-cols-2 gap-3">
                          {step.nodes.map((node) => (
                            <div key={node.label} className={`flex gap-3 items-start p-4 rounded-2xl border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 text-left`}>
                    ...
                    <div className="relative shrink-0">
                      <div
                        className={`relative flex h-36 w-36 md:h-48 md:w-48 items-center justify-center rounded-3xl border border-black/10 dark:border-white/10 ${step.bg} backdrop-blur-xl`}
                        style={{ boxShadow: `0 0 60px -10px ${step.glow}` }}
                      >
                        <step.icon className={`h-16 w-16 ${step.color}`} />
                        <span className="absolute -top-5 -right-5 text-6xl font-black text-black/5 dark:text-white/5 select-none">{step.number}</span>
      ...
      {/* === TECH STACK === */}
      <div className="container relative z-10 max-w-5xl mx-auto mt-32">
        <BlurFade delay={0.1} offset={20}>
          <div className="relative rounded-[2.5rem] border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 backdrop-blur-3xl shadow-2xl overflow-hidden p-10 md:p-16">
            ...
            <div className="relative z-10 text-center mb-10">
              <div className="inline-flex items-center gap-2 rounded-full border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 px-4 py-1.5 text-xs font-semibold mb-5">
                <Cpu className="h-3.5 w-3.5 text-green-500" />
                Powered By
              </div>
              <h2 className="text-3xl md:text-4xl font-extrabold tracking-tight">World-Class AI, End to End</h2>
            </div>
            <div className="relative z-10 grid grid-cols-2 md:grid-cols-4 gap-6">
              {[
                ...
                <div key={item.label} className="flex flex-col items-center text-center gap-3 p-5 rounded-2xl border border-black/5 dark:border-white/5 bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 transition-all group">
      ...
      {/* === CTA === */}
      <div className="container relative z-10 max-w-3xl mx-auto mt-24 text-center">
        <BlurFade delay={0.2} offset={20}>
          <h2 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4">
            See it for Yourself
          </h2>
          <p className="text-lg text-black/60 dark:text-white/60 mb-10">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/login" className="group relative inline-block">
              ...
              <Button size="lg" className="relative h-14 rounded-full bg-black text-white dark:bg-white dark:text-black hover:bg-black/90 dark:hover:bg-white/90 px-10 text-lg shadow-xl font-bold">
                Start for Free
              </Button>
            </Link>
            <Link to="/about">
              <Button variant="outline" size="lg" className="h-14 rounded-full border-black/20 dark:border-white/20 bg-black/5 dark:bg-white/5 px-10 text-lg font-medium hover:bg-black/10 dark:hover:bg-white/10">
